...
export const useLeagueStore = defineStore('league', () => {
  ...
  const { data: scheduleData, pending: schedulePending, error: scheduleError } = useFetch<Game[]>(
    `${API_BASE}?otsl_action=get_league_schedule&season_year=${SEASON_YEAR}`
  )

  const { data: standingsData, pending: standingsPending, error: standingsError } = useFetch<Standing[]>(
    `${API_BASE}?otsl_action=show_standings&season_year=${SEASON_YEAR}`
  )

  const { data: scorersData, pending: scorersPending, error: scorersError } = useFetch<Scorer[]>(
    `${API_BASE}?otsl_action=get_scorers&season_year=${SEASON_YEAR}`
  )
  ...
  const filteredSchedule = computed(() => {
    ...
    return scheduleData.value.filter(
      (g) => g.division_id === scheduleDivisionId.value && g.schedule_type === scheduleType.value
    )
  })
